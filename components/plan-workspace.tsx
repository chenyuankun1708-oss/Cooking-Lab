...
import { buildMealPlan } from "@/lib/meal-plan";
import { encodeMealPlanSharePayload, parseMealPlanLocalState } from "@/lib/meal-plan-codec";
...
export function PlanWorkspace({
  ...
  const [state, setState] = useState<MealPlanLocalStateV1 | undefined>(() => initialPlan ? freshState(initialPlan) : undefined);
  ...
  useEffect(() => {
    ...
    const parsed = parseMealPlanLocalState(raw);
    if (initialPlan) {
      if (parsed && samePlanIdentity(parsed.plan, initialPlan)) {
        queueMicrotask(() => { if (!cancelled) setState(parsed); });
        return () => { cancelled = true; };
      }
      const next = freshState(initialPlan);
      persist(next, setStorageWarning);
      return () => { cancelled = true; };
    }
    if (raw && !parsed) queueMicrotask(() => { if (!cancelled) setStorageWarning(true); });
    if (parsed) queueMicrotask(() => { if (!cancelled) setState(parsed); });
    return () => { cancelled = true; };
  }, [initialPlan]);
    ...
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-16">
      <header className="grid gap-7 border-b border-[var(--line)] pb-9 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button className="focus-ring min-h-11 px-3 font-bold text-[var(--tomato)] hover:underline" onClick={() => { window.localStorage.removeItem(storageKey); setState(undefined); }} type="button">{copy.clear}</button>
        </div>
              ...
              <article className="grid grid-cols-[1fr_auto] items-center gap-4 border-t border-[var(--line)] py-4" key={selection.itemId}>
                <div>
                  ...
                  <label className="mt-1 flex items-center gap-3 text-sm text-stone-600">
                    {copy.servings}
                    <input className="focus-ring h-11 w-20 rounded-[4px] border border-stone-400 bg-[var(--surface-paper)] px-3" min="0.5" max="50" step="0.5" type="number" value={selection.servings} onChange={(event) => rebuild(plan.selections.map((entry) => entry.itemId === selection.itemId ? { ...entry, servings: Number(event.target.value) } : entry))} />
                  </label>
                </div>
      ...
      <nav aria-label={copy.views} className="flex gap-2 overflow-x-auto border-b border-[var(--line)] pb-3">
...
function samePlanIdentity(left: MealPlanV1, right: MealPlanV1): boolean { return left.id === right.id && left.selections.length === right.selections.length && left.selections.every((selection, index) => selection.itemId === right.selections[index]?.itemId && selection.servings === right.selections[index]?.servings); }
...
function unitLabel(unit: MealPlanV1["shopping"][number]["unit"], locale: SupportedLocale): string { const labels = { g: { "zh-CN": "克", en: "g" }, kg: { "zh-CN": "千克", en: "kg" }, ml: { "zh-CN": "毫升", en: "ml" }, piece: { "zh-CN": "个", en: "pc" }, tbsp: { "zh-CN": "大勺", en: "tbsp" }, tsp: { "zh-CN": "小勺", en: "tsp" }, item: { "zh-CN": "份", en: "item" } } as const; return labels[unit][locale]; }
...
const planCopy = {
  "zh-CN": { locale: "zh-CN", eyebrow: "从决定到上桌", title: "把今晚这餐安排清楚", summary: (items: number, minutes: number) => `${items} 项料理，预计协调 ${minutes} 分钟。计划只保存在这台设备的浏览器中。`, items: "今晚的料理", servings: "份数", remove: "移除", share: "复制分享链接", copied: "分享链接已复制。链接只包含公开料理和份数。", clear: "清空计划", views: "计划视图", viewLabels: { shopping: "购物清单", timeline: "准备时间线", cook: "烹饪模式" }, shopping: "需要准备什么", timeline: "什么时候做什么", cook: "一步一步完成", cookNote: "勾选状态和计时仅保存在当前浏览器。关闭页面后可以继续。", cue: "完成信号", startTimer: (minutes: number) => `开始 ${minutes} 分钟计时`, stopTimer: "停止计时", minute: (value: number) => value === 0 ? "现在" : `${value} 分钟后`, duration: (value: number) => `约 ${value} 分钟`, taskKinds: { active: "主动操作", wait: "等待", "prepare-ahead": "提前准备", serve: "上桌" }, emptyEyebrow: "今晚计划", emptyTitle: "还没有安排这顿饭", emptyBody: "先从今晚的推荐、任意料理详情或一份搭配开始。", decide: "决定今晚吃什么", shareWarning: "分享链接无效或包含未发布料理，已安全忽略。", storageWarning: "旧计划或本地数据无法安全读取，已忽略；没有上传任何内容。" },
  en: { locale: "en", eyebrow: "From decision to table", title: "Make tonight's meal manageable", summary: (items: number, minutes: number) => `${items} items, about ${minutes} minutes to coordinate. This plan stays in this browser on this device.`, items: "Tonight's items", servings: "Servings", remove: "Remove", share: "Copy share link", copied: "Share link copied. It contains only public items and servings.", clear: "Clear plan", views: "Plan views", viewLabels: { shopping: "Shopping list", timeline: "Preparation timeline", cook: "Cooking mode" }, shopping: "What to get ready", timeline: "What happens when", cook: "Cook step by step", cookNote: "Checks and timers stay in this browser. You can continue after closing the page.", cue: "Done when", startTimer: (minutes: number) => `Start ${minutes} min timer`, stopTimer: "Stop timer", minute: (value: number) => value === 0 ? "Now" : `In ${value} min`, duration: (value: number) => `About ${value} min`, taskKinds: { active: "Active", wait: "Wait", "prepare-ahead": "Prepare ahead", serve: "Serve" }, emptyEyebrow: "Tonight's plan", emptyTitle: "Nothing is planned yet", emptyBody: "Start from tonight's recommendations, any culinary detail, or a meal pairing.", decide: "Decide tonight", shareWarning: "This share link is invalid or contains an unpublished item, so it was safely ignored.", storageWarning: "An older or invalid local plan could not be read safely and was ignored. Nothing was uploaded." },
} as const;

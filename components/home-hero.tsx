...
export function HomeHero({ items, locale }: { items: readonly HomeHeroItem[]; locale: SupportedLocale }) {
  ...
  return (
    <>
      {header}
      <section className="border-b border-[var(--line)]" aria-labelledby="home-title">
        <div className="mx-auto grid min-h-[calc(100dvh-4rem)] max-w-7xl gap-8 px-4 py-8 sm:px-6 sm:py-10 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] lg:items-center lg:gap-12 lg:py-12">
          <div className="max-w-xl lg:pr-4">
            <p className="editorial-kicker">{item.editorialLine}</p>
            <h1 id="home-title" className="mt-4 max-w-[12ch] text-5xl leading-[1.02] text-stone-950 sm:text-6xl xl:text-7xl">{messages.home.title}</h1>
          ...
          <figure className="min-w-0">
            <div className="relative aspect-[4/3] overflow-hidden rounded-[4px] bg-stone-200 lg:aspect-[3/2]">
              <Image

...
export function SiteFooter({ locale }: { locale: SupportedLocale }) {
  ...
  return (
    <footer className="border-t border-[var(--line)] bg-[var(--surface-story)] px-5 py-10">
      <div className="mx-auto flex max-w-7xl flex-col gap-7 sm:flex-row sm:items-start sm:justify-between">
        ...
        <nav aria-label={messages.footer.label} className="flex flex-col gap-3 text-sm font-semibold text-[var(--tomato)]">
          ...
          <Link className="focus-ring inline-flex min-h-11 items-center hover:underline" href={getLocalizedPath(locale, "/content-rights")}>
            {messages.footer.contentRights}
          </Link>

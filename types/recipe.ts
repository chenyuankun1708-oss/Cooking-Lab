...
export interface Recipe { id:string; slug:string; name:string; description:string; heroImageId?:string; taxonomy:RecipeTaxonomy; flavor:FlavorProfile; culture?:RecipeCulturalContext; servings:number; ingredients:RecipeIngredient[]; nutrition?:Nutrition; cooking:{prepTime:number;cookTime:number;totalTime:number;oil:number;salt:number;addedSugar:number;difficulty:Difficulty}; tools:string[]; cost:{estimated?:number;currency:"CNY";basis:string};steps:RecipeStep[];principles:string[];dataQuality:"demo-estimated" }

...
export function RecipeDiscovery({ recipes, ingredients }: { recipes: Recipe[]; ingredients: Ingredient[] }) {
  ...
  const cuisines = useMemo(() => [...new Set(recipes.map((recipe) => recipe.cuisine))].sort(), [recipes]);
  ...
  const toggle = (key: "availableIngredients" | "availableTools" | "dietaryTags", value: string) => setCriteria((current) => {
    const values = current[key] ?? [];
    return { ...current, [key]: values.includes(value) ? values.filter((item) => item !== value) : [...values, value] };
  });
  const setNumber = (key: keyof RecommendationCriteria, value: string) => setCriteria((current) => ({ ...current, [key]: value ? Number(value) : undefined }));
    ...
    <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
      <div className="grid items-start gap-8 lg:grid-cols-[minmax(17rem,21rem)_1fr]">
        <aside className="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm lg:sticky lg:top-4" aria-labelledby="filter-title">
          ...
          <p className="mt-2 text-sm leading-6 text-stone-600">食材和厨具代表“我拥有的完整集合”，菜谱的必需项需全部满足。</p>
          <div className="mt-5 space-y-5">
            <fieldset><legend className="font-semibold">时间与目标</legend><div className="mt-3 grid grid-cols-2 gap-3">
              ...
              <Select label="最多用油" value={criteria.maxOil} options={[5, 10, 15]} suffix="g" onChange={(v) => setNumber("maxOil", v)} />
        ...
        <div>
          <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-semibold text-emerald-700">即时决策结果</p><h2 className="mt-1 text-3xl font-bold" aria-live="polite">找到 {results.length} 道料理</h2></div><p className="text-sm text-stone-500">营养与价格均为 demo 估算</p></div>
          ...
          {results.length ? <div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{results.map((result) => <RecipeCard key={result.recipe.id} result={result} />)}</div> : <div className="mt-6 rounded-3xl border border-dashed border-stone-300 bg-white px-6 py-16 text-center"><h3 className="text-2xl font-bold">暂时没有同时满足这些条件的料理</h3><p className="mx-auto mt-3 max-w-lg leading-7 text-stone-600">可以放宽时间或预算、减少营养限制，或补充食材和厨具条件。</p><button type="button" onClick={reset} className="mt-6 min-h-11 rounded-xl bg-emerald-700 px-5 font-semibold text-white hover:bg-emerald-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-700">重置筛选</button></div>}
        </div>
      </div>
    </section>
  );
...
function summary(criteria: RecommendationCriteria, ingredients: Ingredient[]): string[] {
  ...
  return [
    ...
    criteria.maxOil !== undefined ? `≤ ${criteria.maxOil} g 油` : undefined,

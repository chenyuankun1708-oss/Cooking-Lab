...
const dishSpecs = [
  {
    ...
    story: {
      ...
      zhClaim: "两份独立书面来源都以鲜柠檬和锅中加热呈现中式餐馆或外卖风格柠檬鸡；其中一份明确采用不油炸路径，另一份另做柠檬酱汁。",
      enClaim: "Two independent written sources use fresh lemon and stovetop cooking for Chinese restaurant or takeout-style lemon chicken; one explicitly avoids deep-frying, while the other builds a separate lemon sauce.",
  ...
  },
  {
    ...
    zhName: "美式中餐西兰花鸡片",
    enName: "American-Chinese Chicken and Broccoli",
    ...
    taxonomy: { origin: { countryId: "united-states" }, cuisine: { cuisineId: "fusion" }, techniqueIds: ["blanch", "stir-fry"], formIds: ["main-dish"], dietaryTagIds: [], browseTagIds: ["quick", "vegetable-rich"] },
    ...
    pairing: { mealRoleIds: ["main"], servingContextIds: ["lunch", "dinner"], cuisineIds: ["fusion"], facets: [{ dimension: "weight", value: "medium" }, { dimension: "temperature", value: "hot" }, { dimension: "texture", value: "crisp" }] },
    preparation: {
      kind: "cooking",
    ...
    story: {
      ...
      zhBoundary: "两份来源都明确处于美式中餐外卖语境；本配方据此标为美国融合料理，数量、文案和状态提示由 Cooking Lab 独立组织。",
      enBoundary: "Both sources explicitly frame the dish in an American-Chinese takeout context; this entry is therefore classified as a U.S. fusion dish, with independently authored quantities, wording, and state cues.",
  ...
  },
  {
    ...
    story: {
      ...
      zhClaim: "两份独立书面来源都把熟鸡肉、脆蔬菜和独立调味汁组合成沙拉式料理；其中一份从现成熟鸡开始，另一份明确将煮熟鸡肉冷却后手撕。",
      enClaim: "Two independent written sources combine cooked chicken, crisp vegetables, and a separate dressing as a salad-style dish; one starts with already-cooked chicken, while the other explicitly cools and shreds freshly cooked chicken.",
  ...
  },
  {
    ...
    story: {
      ...
      zhClaim: "两份独立书面来源都以香茅调味鸡肉并在调味后高温烹调；一份采用炒制，另一份采用烤制，具体腌料组成并不相同。",
      enClaim: "Two independent written sources season chicken with lemongrass and cook it over substantial heat; one stir-fries and the other roasts, and their exact marinade compositions differ.",
  ...
  },
  {
    ...
    zhName: "得州墨西哥风味鸡肉法希塔",
    enName: "Tex-Mex Chicken Fajitas",
    zhDescription: "以得州墨西哥风味为语境，鸡胸、彩椒和洋葱分段煎炒，以青柠、孜然和红椒粉调味，装入温热玉米饼即食。",
    enDescription: "In a Tex-Mex context, chicken breast, bell pepper, and onion are seared in stages with lime, cumin, and paprika, then served in warm corn tortillas.",
    heroAlt: "Chicken fajita strips with peppers, lime, and corn tortillas",
    taxonomy: { origin: { countryId: "united-states" }, cuisine: { cuisineId: "fusion" }, techniqueIds: ["sear", "stir-fry"], formIds: ["main-dish", "staple"], dietaryTagIds: [], browseTagIds: ["quick", "vegetable-rich"] },
    ...
    pairing: { mealRoleIds: ["main", "staple"], servingContextIds: ["lunch", "dinner"], cuisineIds: ["fusion"], facets: [{ dimension: "weight", value: "medium" }, { dimension: "temperature", value: "hot" }, { dimension: "texture", value: "crisp" }] },
    preparation: {
      kind: "cooking",
    ...
    story: {
      ...
      zhClaim: "两份独立书面来源都把鸡肉法希塔组织为调味鸡肉、彩椒、洋葱与温热饼皮的组合；一份分段处理组件，另一份在同一口锅中合并烹调。",
      enClaim: "Two independent written sources organize chicken fajitas around seasoned chicken, peppers, onion, and warm tortillas; one stages the components, while the other cooks them together in one pan.",
      ...
      zhBoundary: "本条目按得州饮食写作证据归为美国融合料理，只交叉核对当代上桌结构，不代表任何餐厅官方配方。",
      enBoundary: "This entry follows the Texas food-writing evidence and is classified as a U.S. fusion dish; it cross-checks a contemporary serving structure and represents no official restaurant formula.",

...
视觉抽样另按料理类型建立 `visual-fidelity:*` 等价类，使主体、配料与过敏原可见性错误只冻结对应视觉风险类，而不会无差别冻结同一 sampled item 代表的全部来源与许可证类别。
...
每个等价类至少有一个真实 sampled item；每个 sampled item 必须留下逐项、全维度、带 verdict 与 findings 的 durable evidence，不能只用批次级 PASS 或虚假 item ID 满足覆盖。sampling auditor 必须与 author 及 primary reviewer 使用不同 actor/run/context；连续批次还必须使用不同 auditor run、context 和 evidence reference，复制同一份审计不能计作第二个 recovery batch。MEDIUM 的逐项双上下文审查不被 sampling 替代。
...
AI-assisted expression 另外要求一对一的 `AiGenerationRecord`。记录必须与当前输出 version 对齐，author 身份必须与所链接 attestation 的 author 完全一致；rights/license、provenance、editorial 三个 PASS 既要覆盖所属 item，也要来自与 author 不同的 reviewer actor/run/context。输入包只能保存 Source、Evidence、ResearchRecord 和 rights assessment 的结构化 ID/hash，不能保存第三方受保护表达，并且每个 Source ID 必须解析到 registry 中的真实 Source。服务路线必须显式声明为 direct 或 gateway：direct 只允许一个 model provider；gateway 必须记录至少一个网关并以唯一 model provider 结束。每一层都要有允许 store、transform、publish、commercialize 的 provider-bound 条款 assessment，并闭合进 UsageDecision。历史运行、prompt、输入或服务链无法可靠恢复时，候选保持 `generated + block`，不得用空记录、推测记录或 derivation 改名绕过。
...
餐厅内容身份也采用声明与审查分离的 fail-closed contract。`restaurantRequirements` 是需要餐厅身份判断的完整集合，必须与 `restaurants` 中的结构化身份一一对应；任何漏项、额外身份或 kind 不一致都会阻止发布。`cooking-lab-reconstruction` 的权利记录只能声明当前 policy 下需要 `factual-culinary` review，不能在 rights registry 内自称已经 PASS。其两个以上来源必须在 closed `ResearchRecord` 中逐一同时标记为 `identity` 与 `preparation` 用途，最终 PASS 只能由当前 artifact version 的独立 `ReviewAttestation` 解析。内容变更后该 attestation 按 artifact fingerprint 自动失效。

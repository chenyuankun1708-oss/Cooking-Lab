...
export function RecipeDiscovery({ recipes, ingredients }: { recipes: Recipe[]; ingredients: Ingredient[] }) {
  ...
  const methods = useMemo(() => [...new Set(recipes.map((recipe) => recipe.cooking.method))].sort(), [recipes]);
      ...
      <div className="grid items-start gap-8 lg:grid-cols-[minmax(17rem,21rem)_1fr]">
        <aside className="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm lg:sticky lg:top-4" aria-labelledby="filter-title">
          ...
          <div className="mt-5 space-y-6">
            ...
            <fieldset><legend className="font-semibold">料理偏好</legend><div className="mt-3 space-y-4"><label className="block text-sm"><span className="mb-1 block text-stone-600">偏好菜系</span><select value={criteria.preferredCuisine ?? ""} onChange={(event) => setCriteria((current) => ({ ...current, preferredCuisine: event.target.value || undefined }))} className="min-h-11 w-full rounded-xl border border-stone-300 bg-white px-2 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-700"><option value="">无偏好</option>{cuisines.map((cuisine) => <option key={cuisine}>{cuisine}</option>)}</select></label><div><p className="text-sm text-stone-600">偏好技法</p><div className="mt-2 flex flex-wrap gap-2">{methods.map((method) => <Toggle key={method} label={method} checked={criteria.preferredMethods?.includes(method) ?? false} onChange={() => toggle("preferredMethods", method)} />)}</div></div><div><p className="text-sm text-stone-600">偏好标签</p><div className="mt-2 flex flex-wrap gap-2">{valuableTags.filter((tag) => recipes.some((recipe) => recipe.tags.includes(tag))).map((tag) => <Toggle key={tag} label={getTagLabel(tag)} checked={criteria.preferredTags?.includes(tag) ?? false} onChange={() => toggle("preferredTags", tag)} />)}</div></div></div></fieldset>
          </div>
        </aside>
        <div>
...
function summary(criteria: RecommendationCriteria, ingredients: Ingredient[]): string[] {
  ...
  return [
    ...
    ...(criteria.preferredMethods ?? []).map((method) => `偏好${method}`),
    ...
    criteria.preferredCuisine ? `偏好${criteria.preferredCuisine}` : undefined,

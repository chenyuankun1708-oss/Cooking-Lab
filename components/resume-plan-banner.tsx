...
export function ResumePlanBanner({ locale }: { locale: SupportedLocale }) {
  ...
  useEffect(() => {
    let cancelled = false;
    let state: ReturnType<typeof parseMealPlanLocalState>;
    try {
      state = parseMealPlanLocalState(window.localStorage.getItem("cooking-lab:meal-plan:v1"));
    } catch {
      state = undefined;
    }
  ...
  return (
    <aside className="border-b border-[var(--line)] bg-[var(--surface-story)]">
      <div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
        <div><p className="font-bold text-stone-950">{locale === "zh-CN" ? "今晚的计划还在这里" : "Tonight's plan is still here"}</p><p className="mt-1 text-sm text-stone-600">{locale === "zh-CN" ? `已安排 ${count} 项料理，购物与步骤状态保存在当前浏览器。` : `${count} ${count === 1 ? "item" : "items"} planned. Shopping and cooking progress stay in this browser.`}</p></div>

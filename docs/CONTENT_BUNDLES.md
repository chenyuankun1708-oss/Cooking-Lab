...
- Every text artifact has a mandatory explicit derivation declaration; missing or extra declarations stop registry construction, so omission cannot silently relabel generated text as factual synthesis. The AI record is artifact-specific and binds the current output version to the actual author actor/run/context, immutable model identity, versioned prompt hash, canonically hashed structured input bundle, closed research records, source-rights assessments, and an explicit service route. Direct-provider routes must contain exactly one model-provider assessment; gateway routes must contain at least one assessed gateway followed by exactly one assessed model provider. Every service-chain node must map one-to-one to a provider-bound terms assessment allowing store, transform, publish, and commercialize, every input Source must resolve in the registered source set, and the UsageDecision must include the full closure. Missing historical provenance is a blocker, not a field to reconstruct by guesswork.
...
- M11 Batch A currently demonstrates this fail-closed path: its 105 AI-assisted text artifacts are classified as `generated`, while unverifiable historical run/prompt/input and NewAPI/upstream terms metadata are intentionally not fabricated. Those 35 candidates remain outside Production. Three regional category profiles are not counted as specific `ProductProfile` records because they have no real brand, producer, vintage/model, or batch; placeholder values are prohibited.

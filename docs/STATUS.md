...
- Batch A 的 35 项候选、双语 Story、明确的 step metadata 和原创本地 Hero 已建立；源条目继续保持 draft，独立的原子发布 projection 与 committed manifest 已准备好，但 Production 仍为 50 项。当前 105 个 AI-assisted 文字 artifact 已如实标为 `generated`；历史 author/run/prompt/input 与 NewAPI→真实上游商业输出条款链不能从可靠记录重建，因此没有事后伪造 `AiGenerationRecord`，对应 UsageDecision 自动 BLOCK。所有文字 artifact 现在必须有显式 derivation declaration；AI 输入使用可重算的 canonical hash，UsageDecision 必须闭合输入、来源、网关与 model-provider 的全部 assessment。泛化栏目页与失效跳转已由直接的非视频机构、出版或专业来源替换。独立复审将 3 个只有家庭/一般料理证据的条目移出 restaurant reconstruction registry，现有 Batch A 仅保留 5 个有相应身份证据的重构；另将 3 个无品牌、生产者和批次的区域类别档案移出具体 `ProductProfile`，M11 的 12 个真实具体产品档案目标仍未满足，不能用占位字段充数。

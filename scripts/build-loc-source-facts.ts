...
assertSafeOutput(outputRoot, allowedOutputParent);
...
function assertSafeOutput(target: string, parent: string): void {
  const rel = relative(parent, target);
  if (!rel || rel.startsWith("..") || resolve(parent, rel) !== target || dirname(target) !== parent) {
    throw new Error(`Unsafe source-fact output path: ${target}`);
  }
  if (existsSync(target) && lstatSync(target).isSymbolicLink()) {
    throw new Error(`Source-fact output path must not be a symlink: ${target}`);
  }
}

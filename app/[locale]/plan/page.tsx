...
import { decodeMealPlanSharePayload } from "@/lib/meal-plan-codec";
...
export default async function PlanPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<RouteSearchParams> }) {
  ...
  const payload = decodeMealPlanSharePayload(query);
  const initialPlan = payload ? buildPublishedMealPlan(payload, locale) : undefined;
  const invalidShareLink = query.has("items") && !initialPlan;
  return <main id="main-content"><SiteHeader locale={locale} currentPath={`/${locale}/plan`} query={query.toString()} /><PlanWorkspace catalog={getPublishedMealPlanCatalog(locale)} initialPlan={initialPlan} invalidShareLink={invalidShareLink} locale={locale} /><SiteFooter locale={locale} /></main>;
}

...
export default async function PairingPage({
  ...
  return (
    <main id="main-content">
      ...
      <article>
        <header className="border-b border-[var(--line)] px-4 pb-12 pt-7 sm:px-6 sm:pb-16 sm:pt-10">
          <div className="mx-auto grid max-w-7xl items-end gap-9 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] xl:gap-14">
            <div className="xl:pb-3">
              <nav aria-label={c.breadcrumb} className="flex flex-wrap items-center gap-2 text-sm text-stone-500">
                <Link className="focus-ring inline-flex min-h-11 items-center font-semibold text-[var(--tomato)] hover:underline" href={decisionState.source === "catalog" ? decisionReturnHref : getLocalizedPath(locale, "/recipes")}>{messages.nav.recipes}</Link>
                <span aria-hidden="true">/</span>
                <Link className="focus-ring inline-flex min-h-11 items-center font-semibold text-[var(--tomato)] hover:underline" href={anchorHref}>{experience.anchor.name}</Link>
                <span aria-hidden="true">/</span>
                <span aria-current="page">{c.breadcrumbCurrent}</span>
              </nav>
              <p className="editorial-kicker mt-8">{c.eyebrow}</p>
              <h1 className="mt-3 max-w-[18ch] text-5xl leading-[0.98] text-stone-950 sm:text-6xl">{c.title(experience.anchor.name)}</h1>
              ...
              {hasContext ? (
                <DecisionContextSummary
                  ...
                  locale={locale}
                  surface="pairing"
                  anchorIsRecipe={experience.anchorIsRecipe}
                />
              ) : null}
              {experience.appliedRelaxationIds.length ? (
                <div className="mt-6 border-l-2 border-[var(--tomato)] pl-4 text-sm leading-6 text-stone-700">
        ...
        <section className="mx-auto max-w-6xl px-4 py-14 sm:px-6 sm:py-20" aria-labelledby="recommended-meal-title">
          <p className="editorial-kicker">{c.recommendation}</p>
          <h2 id="recommended-meal-title" className="mt-3 max-w-3xl text-4xl leading-tight text-stone-950 sm:text-6xl">{c.recommendedTitle}</h2>
                ...
                {experience.relaxationOptions.length ? (
                  <div className="mt-6 flex flex-wrap gap-3">
                    {experience.relaxationOptions.map((option) => (
                      <Link
                        className="focus-ring inline-flex min-h-11 items-center rounded-[4px] border border-stone-950 px-4 py-2 text-sm font-semibold text-stone-950 transition hover:border-[var(--tomato)] hover:text-[var(--tomato)]"
        ...
        {experience.alcoholicAlternative ? (
          <section className="border-t border-[var(--line)] bg-[var(--surface-story)] py-12 sm:py-16" aria-labelledby="alcoholic-alternative-title">
            <div className="mx-auto max-w-6xl px-4 sm:px-6">
              <p className="text-sm font-semibold text-[var(--tomato)]">{c.optional}</p>
              <h2 id="alcoholic-alternative-title" className="mt-2 text-3xl font-bold text-stone-950 sm:text-4xl">{c.alcoholicTitle}</h2>
              <p className="mt-3 max-w-3xl leading-7 text-stone-700">{c.alcoholicNote}</p>
        ...
        {experience.anchorIsAlcoholic && experience.nonAlcoholicAlternative ? (
          <section className="border-t border-[var(--line)] py-12 sm:py-16" aria-labelledby="non-alcoholic-alternative-title">
            <div className="mx-auto max-w-6xl px-4 sm:px-6">
              <p className="text-sm font-semibold text-[var(--tomato)]">{c.alternativeDrink}</p>
              ...
              <Link className="focus-ring mt-6 inline-flex min-h-11 items-center font-semibold text-[var(--tomato)] hover:underline" href={appendQueryToHref(experience.nonAlcoholicAlternative.href, routeQuery)}>
